...
- **按能力拆分**：应用入口、问答执行、文档知识构建、知识增强、Python 工具连接和模型服务连接各有明确归属，新增能力时更容易找到应该修改的位置
- **协作方向清晰**：问答、知识构建和知识增强只使用自己需要的能力，由应用入口统一把各部分连接起来，避免模块之间相互牵扯
- **统一组装与启动**：拆分后的 Java 模块仍由同一个后端应用统一配置和启动，既保留清晰边界，也不会增加不必要的部署负担
- **外部连接集中管理**：模型服务与 Python 工具服务分别由独立模块连接，超时、取消和调用失败在连接边界统一处理，不让这些细节散落到业务流程中
- **公共能力按需复用**：统一返回、异常处理、缓存、任务锁定、占用续期和唯一编号等能力集中封装，聊天与文档处理可以按需使用
- **接口说明保持同步**：接口说明跟随接口定义生成，便于调试和对接，也减少功能已经变化、说明还停留在旧版本的情况

:::info 分层架构详解
Nexus Agent Pro 按照**每部分负责什么事情**来组织工程。随着问答和知识处理能力增加，业务工程已经进一步拆成 **6 个模块**，分别承担应用入口、问答执行、文档知识构建、知识增强、Python 工具连接和模型服务连接。

- **业务工程（nexus-agent-business）**：集中组织这 6 个模块。聊天模块负责接收请求、管理会话和接好各项能力；查资料与回答、处理文档、构建内容关系和分层摘要，则交给各自的模块。
- **通用能力（nexus-agent-common）**：提供统一返回结果、异常处理、分页和数据处理等公共能力，减少各处重复编写相同逻辑。
- **基础组件（nexus-agent-\*-framework）**：提供缓存、任务锁定、任务占用续期、延迟处理和唯一编号等支撑，供聊天与文档处理共同使用。
- **Python 工具服务（nexus-agent-rag-tools）**：独立运行，提供文档解析、材料相关程度打分、内容关系抽取和分层摘要构建等能力。Java 负责决定查哪些资料、采用哪些依据以及怎样组织回答。

这样的拆分让小伙伴更容易顺着业务找到对应部分：调整文档处理方式，重点看知识构建模块；改进查资料和回答的过程，重点看问答执行模块；处理模型连接，则集中在模型服务连接模块。各部分通过约定好的接口配合，减少一次改动需要同时理解的内容。

这里的 Java 模块拆分属于同一个后端应用内部的职责划分，由聊天模块统一组装和启动，并不需要把这 6 个模块分别部署成 6 个服务。
:::
...
| 模块 | 负责什么 | 主要内容 |
| :--- | :--- | :--- |
| nexus-agent-business-chat | 应用入口与业务组织 | 接收聊天和管理请求，处理身份校验、会话与记忆、对话记录，并连接各模块、统一启动应用 |
| nexus-agent-rag-runtime | 问答执行 | 理解问题、确定可用资料范围、从多种途径查找资料、筛选回答依据、组织答案与引用，以及安排工具调用 |
| nexus-agent-knowledge-indexing | 文档知识构建 | 管理文档处理任务，组织解析与内容切分，保存章节、表格等结构，并建立用于查找资料的数据 |
| nexus-agent-knowledge-augmentation | 知识增强 | 从已有文档内容中构建关系网络和分层摘要，检查构建结果，并为问答提供相应的查找能力 |
| nexus-agent-rag-tools-http | 连接 Python 工具服务 | 将处理请求交给 Python 工具，接收结果，统一处理连接超时和调用错误 |
| nexus-agent-model-http | 连接模型服务 | 发送对话请求、接收逐步生成的回答，将文本转换为便于按含义查找的数据，并处理连接、取消和错误 |

这些业务模块共同使用的通用能力、基础组件，以及独立的 Python 工具服务如下：

| 模块或模块组 | 负责什么 | 主要内容 |
| :--- | :--- | :--- |
| nexus-agent-common-frame | 通用基础约定 | 统一返回结果、异常处理、公共状态与工具 |
| nexus-agent-common-web | 通用应用支持 | 分页、数据字段自动填充、日期与数据格式处理、接口文档支持 |
| nexus-agent-id-generator-framework | 生成唯一编号 | 为多实例运行下的业务记录生成唯一编号 |
| nexus-agent-redis-tool-framework | 通用缓存操作 | 统一缓存读写和缓存名称管理 |
| nexus-agent-redisson-framework | 多实例任务协作 | 任务锁定、占用续期、延迟队列和重复执行限制 |
| nexus-agent-rag-tools | Python 工具能力 | 执行文档解析、材料相关程度打分、内容关系抽取和分层摘要构建 |

其中，**连接模块负责把请求送出去、把结果接回来，业务模块负责决定这些结果怎样使用**。例如，Python 可以给候选材料打分，但哪些材料最终用于回答、答案引用哪些原文，仍由 Java 问答流程决定。
