...
description: 深度拆解 Super Agent 的核心架构设计——从三层执行器体系、RAG 前置编排引擎、双通道混合检索、文档处理流水线到会话记忆管理，每一个模块的设计思路和工程实现细节都在这里。
keywords: [Super Agent, 架构设计, 三层执行器, RAG, 前置编排, 双通道检索, 文档处理, 会话记忆, MCP, Skills, Agent]
---
...
partition "  <color:#2563EB><b>PHASE 1</b></color>  <color:#475569>会话记忆加载</color>  " {
  :  从 MySQL 加载会话记忆\n<color:#64748B>无记忆 / 滑动窗口 / 摘要压缩</color>  ;
}

partition "  <color:#2563EB><b>PHASE 2</b></color>  <color:#475569>RAG 前置编排</color>  " {
  :  LLM 意图分析 → 问题改写  ;
...
if (<b>执行器选择</b>) then (<color:#2563EB><b>知识问答</b></color>)
  ...
  partition "  <color:#2563EB><b>PHASE 3</b></color>  <color:#475569>RAG 检索 + 生成</color>  " {
    :  双通道混合检索\n<color:#64748B>PGVector + Elasticsearch → RRF 融合</color>  ;
    :  证据评估与预算控制  ;
  ...
  partition "  <color:#D97706><b>PHASE 3</b></color>  <color:#475569>ReAct Agent</color>  " {
...
partition "  <color:#2563EB><b>PHASE 4</b></color>  <color:#475569>SSE 流式输出</color>  " {
  fork
    :  正文分片推送 + 引用来源  ;
  fork again
...
partition "  <color:#D97706><b>Agent 初始化</b></color>  " {
  ...
  :  ReactAgent 启动\n<color:#64748B>并行工具执行（最多 4 个）· Checkpoint 持久化到 MySQL · 应用重启后可恢复对话</color>  ;

  :  加载 Checkpoint\n<color:#64748B>从 MySQL 恢复对话状态（如有），支持断点续聊</color>  ;
}
...
partition "  <color:#D97706><b>推理循环</b></color>  <color:#475569>Thought → Action → Observation</color>  " {

  repeat

    :  <color:#6D28D9><b>Thought</b></color>\nLLM 分析当前状态，决定下一步  ; <<#EDE9FE>>

    if (需要调用工具？) then (<color:#059669>是</color>)

      :  <color:#FFFFFF><b>调用工具</b></color>\n<color:#FEF3C7>Tavily 联网搜索等</color>  ; <<#D97706>>
...
:  Checkpoint 持久化\n<color:#64748B>保存对话状态到 MySQL</color>  ;
...
:  <b>用户消息进入编排器</b>\n<color:#64748B>ChatPreparationOrchestrator</color>  ; <<#3B82F6>>

partition "  <color:#2563EB><b>Step 1</b></color>  <color:#475569>会话记忆加载</color>  " {
  :  从 MySQL 加载历史记忆\n<color:#64748B>按策略组装上下文</color>  ;
}

partition "  <color:#2563EB><b>Step 2</b></color>  <color:#475569>LLM 会话关系解析</color>  " {
...
partition "  <color:#2563EB><b>Step 3</b></color>  <color:#475569>歧义检测</color>  " {
  if (信息是否充分？) then (<color:#DC2626>不充分</color>)
    :  <color:#FFFFFF><b>生成澄清问题</b></color>\n<color:#FEE2E2>要求用户补充信息</color>  ; <<#DC2626>>
    :  产出执行计划\n<color:#64748B>mode = CLARIFY</color>  ;
    stop
  else (<color:#059669>充分</color>)
  endif
}

partition "  <color:#2563EB><b>Step 4</b></color>  <color:#475569>子问题拆分</color>  " {
  :  复合问题拆分\n<color:#64748B>单轮最多 4 个子问题</color>  ;
}

partition "  <color:#2563EB><b>Step 5</b></color>  <color:#475569>知识域收缩</color>  " {
  :  分析每个子问题所属知识域\n<color:#64748B>缩小检索范围</color>  ;
}

if (路由方向？) then (<color:#2563EB>匹配知识库</color>)
  :  产出执行计划\n<color:#64748B>mode = RAG_CHAT</color>  ;
else (<color:#D97706>开放式问题</color>)
  :  产出执行计划\n<color:#64748B>mode = REACT_AGENT</color>  ;
endif

:  <color:#FFFFFF><b>输出 ConversationExecutionPlan</b></color>\n<color:#DBEAFE>改写问题 + 子问题列表 + 知识域 + 执行模式</color>  ; <<#3B82F6>>
...
partition "  <color:#2563EB><b>双通道检索</b></color>  <color:#475569>并行执行</color>  " {
  ...
  fork
    :  <color:#FFFFFF><b>向量检索</b></color>\n<color:#D1FAE5>PGVector</color>\n<color:#ECFDF5>最低相似度 ≥ 0.45</color>  ; <<#0D9488>>
  fork again
    :  <color:#FFFFFF><b>关键词检索</b></color>\n<color:#FEF3C7>Elasticsearch</color>\n<color:#FFFBEB>相对阈值 ≥ 0.35</color>  ; <<#D97706>>
  end fork

  :  RRF 融合排序\n<color:#64748B>Reciprocal Rank Fusion — 按排名倒数法融合两路结果</color>  ;

  if (启用 Rerank？) then (<color:#059669>是</color>)
    :  外部 Rerank 精排\n<color:#64748B>SiliconFlow 兼容协议</color>  ;
  else (<color:#94A3B8>否</color>)
  endif

  :  Parent-Child 块聚合\n<color:#64748B>检索用 Child 小块保证命中率\n回答用 Parent 大块保证上下文完整</color>  ;
}
...
:  <b>用户上传文档</b>\n<color:#64748B>PDF / Word / TXT 等</color>  ; <<#3B82F6>>
...
partition "  <color:#2563EB><b>阶段四</b></color>  <color:#475569>切块与索引构建</color>  " {
  :  按策略组合执行切块\n<color:#64748B>Parent-Child 层级结构</color>  ;

  :  Embedding 向量化\n<color:#64748B>阿里云 text-embedding-v4</color>  ;
...
partition "  <color:#2563EB><b>记忆加载</b></color>  <color:#475569>从 MySQL 读取</color>  " {
