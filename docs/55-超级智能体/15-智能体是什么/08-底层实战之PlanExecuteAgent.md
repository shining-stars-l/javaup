...
:::info 全局状态设计原因
为什么要设计结构化的全局状态（OverAllState）？因为Plan & Execute是**多轮、非线性**的。不像ReAct那样简单地堆叠消息，这里需要明确记录：
- 这一轮规划了什么任务
- 每个任务的执行结果
- 批判给出了什么反馈
...
:::caution 上下文压缩注意事项
压缩不是"摘要"，是**状态裁剪**——保留对后续决策有用的信息，去掉冗余。多轮迭代后，消息会越来越多，可能超出模型上下文限制。需要在每轮批判未通过后检查是否需要压缩，否则后续迭代可能因超出窗口而失败。
:::
...
:::tip Plan & Execute 自我修正能力
Agent会自动识别执行中的问题并重新执行。批判阶段是 Plan & Execute 的核心——它决定了Agent是继续迭代还是输出结果，实现了真正的"自我修正"能力。
:::
