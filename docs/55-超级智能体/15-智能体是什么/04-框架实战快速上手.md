...
- **文档对应模块**：`super-ai/ai-example/ai-example-spring-ai-alibaba`
- **接入模型**：DeepSeek `deepseek-chat`，服务端口 `7089`
...
```plantuml title="多轮对话记忆原理：threadId 是记忆的钥匙" width="100%"
@startuml
skinparam backgroundColor #F8FBFD
skinparam roundcorner 18
skinparam shadowing false
...
> **生产化建议**：`ResettableMemorySaver` 本质上仍然是内存实现，应用重启后历史会丢失。换成 `MysqlSaver` 或 `PostgresSaver` 时，调用侧的 `threadId` 约定仍然可以保留。
...
```plantuml title="Hook 与 Interceptor 在 Agent 生命周期中的执行位置" width="100%"
@startuml
skinparam backgroundColor #F8FBFD
skinparam roundcorner 18
skinparam shadowing false
...
Service 里的调用做了两层防御，防止模型偶尔输出 Markdown 代码块格式：
...
```plantuml title="SSE 流式推送：chunk 逐字传递的完整链路" width="100%"
@startuml
skinparam backgroundColor #F8FBFD
skinparam roundcorner 18
skinparam shadowing false
