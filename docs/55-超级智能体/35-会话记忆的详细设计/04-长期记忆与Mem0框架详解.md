...
```plantuml title="Mem0的架构组成" width="45%" align="left"
@startuml
...
rectangle "应用接入层\nAI 应用 / Agent\n提交对话，发起记忆检索" as APP #E0F2FE

rectangle "Mem0 协调层\n不直接充当数据库\n统一 API、事实提取、去重合并\n检索排序、生命周期管理" as CORE #DBEAFE

rectangle "模型能力层\nLLM：记忆提取、冲突判断、查询理解\nEmbedding 模型：文本向量化" as MODEL #FFF7ED

rectangle "记忆存储层\n向量数据库：Qdrant、PGVector、Chroma\n图数据库：Neo4j\nSQLite：变更历史与本地状态" as STORE #ECFDF5

APP -down-> CORE : add / search
CORE -down-> MODEL : 调用模型能力
MODEL -down-> STORE : 写入与检索
...
```plantuml title="Mem0记忆添加流程" width="50%" align="left"
@startuml
!theme plain
...
skinparam HyperlinkColor #2563EB
...
skinparam activity {
  ...
  DiamondBorderColor #60A5FA
  DiamondFontColor #1D4ED8
}
...
```plantuml title="Mem0记忆检索流程" width="40%" align="left"
@startuml
!theme plain
