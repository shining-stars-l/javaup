...
title: "双通道检索获得结果后的后续处理(父块提升)：elevateToParentBlocks详解"
sidebar_label: "双通道检索获得结果后的后续处理(父块提升)"
pagination_label: "双通道检索获得结果后的后续处理(父块提升)"
description: "讲解 RAG 检索流程中的父块提升环节，包括 elevateToParentBlocks 方法的分组查询、buildParentEvidenceDocument 构建父块证据、aggregateParentScore 分数计算和证据排序规则。内容进一步围绕父块分数等关键主题展开。通过原理拆解、实现步骤与适用场景说明相…"
