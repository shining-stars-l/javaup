...
title: "执行器注册表与模式分发机制：ConversationExecutor、ExecutionMode详解"
sidebar_label: "执行器注册表与模式分发机制"
pagination_label: "执行器注册表与模式分发机制"
description: "从 BusinessChatService 的 buildConversationExecution 出发，讲解执行器接口设计、执行模式枚举、注册表自动装配，以及当用户选择\"当前文档问答\"时，系统如何把请求路由到 RagChatExecutor。内容进一步围绕执行器注册表、ConversationExecutor、E…"
