...
在原生 SDK 里，这一步其实是需要我们自己来定义字段和调用 `createCollection`。在 Spring AI 方案里，只要配置了 `initialize-schema: true`，框架启动时会自动完成：
...
也就是说，Milvus 的原生 SDK 需要手写的 `createCollection()`、`createIndex()`、`loadCollection()`，在这个项目里收敛成了一个入口。
...
虽然这里没手写 Schema，但底层字段还是存在的。Spring AI 的 Milvus store 会自动创建类似这样的结构：
...
- Spring AI 用 `Document` 的 `List` 表示批量插入，本质上都是批量写入，不是一条一条插。
...
Milvus 的原生 SDK 版需要显式调用加载Collection到内存中。
...
- `searchParamsJson("{\"ef\":64}")`
  这就是原生 SDK 版 `searchParams(Map.of("ef", ...))` 的 Spring AI 写法。
  ...
  写法虽然从 `Map` 变成了 JSON 字符串，但控制的还是同一个东西：HNSW 检索阶段的搜索宽度。
...
### `outputFields` 在 Spring AI 里怎么体现
...
它的作用是告诉 Milvus：除了相似度分数之外，我还希望把哪些标量字段一起带回来。

在当前 Spring AI 项目里，这一步没有消失，而是变成了“由框架统一处理”。Spring AI 默认会把这些内容带回来：

- 文档主键 `id`
- 文本内容 `content`
- 元数据 `metadata`

然后项目代码再把它整理成更适合接口返回的结构。对应代码是：
...
- 原生 SDK：你手动指定 `outputFields`
- Spring AI：自动返回 `id + content + metadata`
- 当前项目：再从 `metadata` 中拆出 `docId`、`category`，整理成最终接口结果

### `annsField("vector")` 在 Spring AI 里怎么体现
...
它表示：在名为 `vector` 的向量字段上执行相似度检索。

在当前 Spring AI 项目里，这个信息被前移到了配置层，不需要每次搜索都重复写。对应配置是：
...
也就是说，原生 SDK 里的 `annsField("vector")`，在 Spring AI 项目里变成了“统一配置一次，后续检索自动复用”的写法。
