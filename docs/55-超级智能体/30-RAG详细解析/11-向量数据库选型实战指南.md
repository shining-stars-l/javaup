...
<img src="/img/super-ai/rag/milvus界面.png" alt="讲解" width="100%" />
...
相比原生 Milvus SDK 方案，这里最大的变化是：**你不再手写 createCollection / createIndex / loadCollection，而是通过 Spring AI 的 starter + 配置驱动完成自动初始化**。
...
你可以把它理解成“文本在向量空间里被展开成多长的一串数字”。这个长度不是随便填的，而是由模型本身决定的。当前示例使用的是 `Qwen/Qwen3-Embedding-8B`，它输出 4096 维，所以这里必须配置成 4096。
...
当前配置的是 `HNSW`。如果你是第一次做 RAG 项目，可以把它理解成“默认优先选它，通常不会出大问题”的那种配置。原因主要有三个：

- 检索速度快，适合交互式问答
- 召回率高，通常能把真正相关的 chunk 找回来
- 工程上很成熟，已经被大量文本检索场景验证过

当然，HNSW 也不是没有代价。它的典型代价是内存占用会比较高，尤其是在向量维度高、数据量大的时候更明显。但如果你现在是在做一个普通的 RAG 示例，或者是百万级以内的数据量，它依然是很自然的第一选择。
...
如果你后面遇到“召回率不够”或者“建索引太慢”的问题，调参通常也是从这两个参数开始。
...
如果你后面发现“结果大致对，但偶尔漏召回”，`ef` 往往就是最值得先试着调大的一个参数。

这里还有一个和原生示例不一样的点：我们把 `auto-id` 设成了 `false`。
...
在原生 SDK 里，这一步需要你自己定义字段和调用 `createCollection`。在 Spring AI 方案里，只要配置了 `initialize-schema: true`，框架启动时会自动完成：
...
public void recreateCollection() {
    ...
    // Spring AI 会在这里内部执行 createCollection、createIndex、loadCollection
    milvusVectorStore.afterPropertiesSet();
}
```

也就是说，文档里原生 SDK 需要手写的 `createCollection()`、`createIndex()`、`loadCollection()`，在这个项目里收敛成了一个入口。

如果你想手动重建 Collection，可以直接调这个接口：
...
虽然这里没手写 Schema，但底层字段还是存在的。Spring AI 的 Milvus store 会帮你创建类似这样的结构：

- `id`
  主键字段。当前项目因为 `auto-id: false`，这里使用的是我们传入的 chunkId。
- `content`
  原文内容字段，存 chunk 文本。
- `metadata`
  JSON 字段，存 `docId`、`category`、`source` 等元数据。
- `embedding`
  向量字段，维度是 4096。

和原生 SDK 手写 `doc_id`、`category` 这种独立标量字段相比，这个项目选择把它们统一收口到 `metadata` JSON 字段中。好处是：

- Java 端代码更简洁
- 扩展 metadata 更方便
- Spring AI 的 filterExpression 可以直接映射到 Milvus 的 JSON 过滤表达式

### 插入数据

原生 SDK 的写法是：`List<JsonObject>`，每个 `JsonObject` 代表一行数据。

Spring AI 的写法换成了：`List<Document>`。框架会自动帮你做三件事：

1. 调用 Embedding 模型，把文本转成 4096 维向量
2. 按配置映射到 `content / metadata / embedding / id`
3. 调用 Milvus 完成批量写入
...
这段代码背后对应的关键点，和原生 SDK 版是一样的：

- 原生 SDK 用 `JsonObject` 的 `List` 表示批量插入，Spring AI 用 `Document` 的 `List` 表示批量插入，本质上都是批量写入，不是一条一条插。
- 原生 SDK 里向量字段的值是 `List<Float>`；Spring AI 里这一步被隐藏了，框架会先调用 Embedding 模型，再把返回的向量写进 Milvus。
- 向量维度依然必须一致。如果 Embedding 输出 4096 维，而 Milvus Collection 配成别的维度，插入时照样会报错。
- 真实项目里也应该尽量批量写入。`vectorStore.add(List<Document>)` 底层就是批量处理，性能比逐条写更好。
...
- `M = 16`
- `efConstruction = 256`
- 给 `category` 单独建 `TRIE` 索引

Spring AI 方案里，这些参数没有消失，而是收敛到了配置里：
...
原生 SDK 版里你会看到给 `category` 单独建 `TRIE` 索引。当前这个 Spring AI 项目没有这样配，原因是这里的 `category` 放在 `metadata` JSON 中，而不是独立字段。

这不是说过滤不能做，而是表示这个 demo 选择了**“metadata JSON + 过滤表达式”**这种更贴近 Spring AI 的写法。如果你后面有很强的过滤性能诉求，完全可以改成原生 Milvus 独立字段模式，再给高频过滤字段单独建标量索引。

### 加载Collection到内存（检索前必须加载）

这是原文里一个非常关键、而且特别容易被改写时漏掉的点：**Milvus 检索是在内存中完成的，所以 Collection 必须先 load，才能检索。**
...
Spring AI 版不是不需要这一步，而是这一步被框架自动做了。
...
你如果想确认它真的已经加载好了，不要靠猜，可以直接调用这个接口：
...
这段代码里，原生 SDK 版本里那些关键检索参数都还有对应物：

- `topK`
  还是表示返回多少条最相似结果。RAG 里一般取 3~10。
- `searchParamsJson("{\"ef\":64}")`
  这就是原生 SDK 版 `searchParams(Map.of("ef", ...))` 的 Spring AI 对应写法。
- `similarityThreshold`
  Spring AI 提供的额外能力，用来截断过低相关度的结果。
- `query(query)`
  你传的是文本，不是向量。Spring AI 会自动完成“query -> embedding -> Milvus 检索”。
...
这段代码最后会被 Spring AI 转换成 Milvus 原生的 filter 表达式，大致等价于：
...
3. 启动项目模块：
   `mvn -pl ai-example/ai-example-rag/ai-example-spring-ai-rag-milvus spring-boot:run`
...
原生 SDK 版强调的是：你要亲手看到 `JsonObject`、`createCollection`、`createIndex`、`loadCollection`、`searchParams(ef)` 这些底层细节。

Spring AI 版强调的是：你在保留这些关键参数的前提下，把重复样板代码交给框架，自己主要维护：
...
所以这两种写法不是互相冲突，而是关注点不同：

- 想学 Milvus 底层 API，原生 SDK 版更直观
- 想做真实 Java RAG 项目，Spring AI 版更接近生产代码
