...
- `query`：用户的检索词
- `mode`：检索模式，取值是 `DENSE_ONLY`、`SPARSE_ONLY`、`HYBRID`
- `category`：可选的分类过滤条件

### 核心点二：应用层统一分发模式，别把判断逻辑散在 Controller 里

真正的模式分发放在 `HybridSearchService` 里，这样控制器会很薄，职责也更清楚：
...
### 核心点三：双写时必须保证两个系统里的主键一致

双系统方案真正容易踩坑的地方，不在 RRF，而在**两路结果最后能不能对上同一篇文档**。
...
所以如果你传进来的是 `redis-7-io-model` 这种业务编码，示例会先把它稳定映射成 UUID，然后再同时给 PGVector 和 ES 用。
...
public List<SearchResultItem> hybridSearch(String queryText, String category) {
    ...
    return rrfFusion(vectorDocs, keywordResults, finalTopK);
}
```
...
这里你可以重点记一句话：**混合检索这一步最重要的不是分数，而是排名。**

### 配置层也有两个细节别忽略

第一，Elasticsearch 的账号密码最好做成可选配置，没开鉴权时不要强制写死：
...
第二，ES 这一路是全文检索，所以中文场景里一般会优先配 IK 分词器；如果本地环境没装 IK，示例里还做了 `standard` 的兜底，这样第一次跑项目时不至于直接卡死在建索引这一步。
