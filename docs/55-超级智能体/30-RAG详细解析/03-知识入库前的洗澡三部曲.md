...
public class PdfReaderStrategy implements DocumentReaderStrategy {
    ...
    public List<Document> read(File file) throws IOException {
        PdfDocumentReaderConfig config = PdfDocumentReaderConfig.builder()
                .withPageTopMargin(50)      // 忽略顶部50单位（跳过页眉）
                .withPageBottomMargin(50)   // 忽略底部50单位（跳过页脚）
...
public class HtmlReaderStrategy implements DocumentReaderStrategy {
    ...
    public List<Document> read(File file) throws IOException {
        JsoupDocumentReaderConfig config = JsoupDocumentReaderConfig.builder()
                .selector("article, .content, main")  // 只提取正文区域
                .charset("UTF-8")
                .includeLinkUrls(false)  // 不包含链接
                .build();
...
public class MarkdownReaderStrategy implements DocumentReaderStrategy {
    ...
    public List<Document> read(File file) throws IOException {
        MarkdownDocumentReaderConfig config = MarkdownDocumentReaderConfig.builder()
                .withHorizontalRuleCreateDocument(true)  // 遇到分割线创建新Document
...
有了各种策略类，还需要一个选择器来根据文件类型选择合适的策略：
...
public class DocumentReaderSelector {
    ...
    public DocumentReaderSelector(List<DocumentReaderStrategy> strategies) {
...
Spring会自动注入所有实现了`DocumentReaderStrategy`接口的Bean，遍历找到能处理的就行。
...
public class DocumentPreprocessService {
    ...
    private final DocumentReaderSelector readerSelector;

    public DocumentPreprocessService(DocumentReaderSelector readerSelector) {
    ...
    public List<Document> process(File file) {
        try {
            ...
            List<Document> docs = readerSelector.read(file);
     ...
     * 批量处理文件夹下的所有文档
     */
    public List<Document> processDirectory(File directory) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                allDocs.addAll(processDirectory(file));  // 递归处理子目录
            } else {
                try {
                    allDocs.addAll(process(file));
                } catch (Exception e) {
                    log.warn("跳过无法处理的文件: {}", file.getName());
                }
            }
        }
