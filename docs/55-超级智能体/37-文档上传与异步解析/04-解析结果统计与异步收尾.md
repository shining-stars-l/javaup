...
title: "解析结果统计与异步收尾：countHeadings、extractParagraphs详解"
sidebar_label: "解析结果统计与异步收尾"
pagination_label: "解析结果统计与异步收尾"
description: "接续结构节点提取之后，逐步拆解 parse() 方法的后半段：标题计数、段落切分、token 估算、结构等级与内容质量评估，以及回到 handleParseRoute 后的解析文本上传、结构节点落库、导航产物同步和解析完成日志，每一步都贴出关键源码并配合讲解。内容进一步围绕countHeadings、extractP…"
