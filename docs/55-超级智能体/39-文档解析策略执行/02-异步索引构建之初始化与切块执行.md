...
title: "异步索引构建：初始化与切块执行：handleIndexBuild、buildParentBlocks详解"
sidebar_label: "异步索引构建：初始化与切块执行"
pagination_label: "异步索引构建：初始化与切块执行"
description: "深入拆解 Kafka 消费端 handleIndexBuild 方法的异步执行流程前半段：任务状态初始化、解析文本读取、Parent-Child 双层流水线切块策略执行，包括结构切块、递归切块、语义切块、LLM 切块四种策略的源码讲解。内容进一步围绕异步索引构建、DocumentAsyncProcessService…"
