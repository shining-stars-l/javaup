...
title: "四种切块策略：executePipeline、流水线执行引擎、结构切块、递归切块、语义切块详解"
sidebar_label: "四种切块策略详解"
pagination_label: "四种切块策略详解"
description: "深入拆解 executePipeline 流水线执行引擎，以及结构切块、递归切块、语义切块、LLM 切块四种策略的完整实现：包括 DocumentLineClassifier 标题识别器、recursiveSplit 四级降级拆分、semanticSplit Jaccard 相似度切分、llmSplit 大模型智能切…"
