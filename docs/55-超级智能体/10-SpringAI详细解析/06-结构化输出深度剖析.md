...
原因很简单：流式输出是一点一点返回的，没法保证拿到完整的JSON再解析。如果需要结构化输出，必须用同步调用（call）。
...
如果报JSON解析错误，先打印原始响应看看：

```java
String rawResponse = chatClient.prompt("xxx").call().content();
...
确保用`@JsonPropertyDescription`给字段加描述，描述越详细越好。

### 3. 模型不遵守格式

有些小模型可能不太遵守JSON Schema的约束，可以尝试：
- 在system提示词里再强调一次格式要求
- 换用能力更强的模型
- 用Few-shot示例来引导
