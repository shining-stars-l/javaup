...
title: "文档导航路由与执行模式判定：DocumentQuestionRouter、route详解"
sidebar_label: "文档导航路由与执行模式判定"
pagination_label: "文档导航路由与执行模式判定"
description: "详细讲解 DocumentQuestionRouter.route 方法完整流程，包括输入准备、统一意图识别调用、三种执行模式（GRAPH_ONLY、GRAPH_THEN_EVIDENCE、RETRIEVAL）的分支判定逻辑，以及显式编号解析。内容进一步围绕ExecutionMode、detectQuestionIn…"
