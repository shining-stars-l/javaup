...
title: "执行计划后处理与上下文同步：prepareExecutionPlan、runnableConfig详解"
sidebar_label: "执行计划后处理与上下文同步"
pagination_label: "执行计划后处理与上下文同步"
description: "讲解 BusinessChatService#prepareExecutionPlan 方法的完整流程，包括 Agent 问题渲染、文档范围同步刷新、runnableConfig 上下文回写，以及 debugTrace 的重建。内容进一步围绕buildAgentQuestion、refreshSessionScope…"
