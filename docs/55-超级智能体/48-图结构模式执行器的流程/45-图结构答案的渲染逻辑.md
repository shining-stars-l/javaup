...
title: "图结构答案的渲染逻辑：GraphAnswerRenderer、renderGraphAnswer详解"
sidebar_label: "图结构答案的渲染逻辑"
pagination_label: "图结构答案的渲染逻辑"
description: "详细讲解 GraphAnswerRenderer 如何将结构图查询结果渲染为自然语言答案，包括相邻章节渲染、子章节列表渲染和兜底处理逻辑。内容进一步围绕renderGraphAnswer、renderGraphOnly、renderAdjacency、renderChildren、答案渲染等关键主题展开。"
