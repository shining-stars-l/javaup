...
title: "Bean生命周期与依赖注入：@PostConstruct、BeanPostProcessor详解"
sidebar_label: "Bean生命周期与依赖注入"
pagination_label: "Bean生命周期与依赖注入"
description: "Bean五个生命周期阶段:实例化/初始化/注册/使用/销毁;@PostConstruct与@PreDestroy;BeanPostProcessor后置处理;Aware接口回调;InitializingBean与DisposableBean;作用域singleton/prototype。内容进一步围绕Bean生命周期…"
