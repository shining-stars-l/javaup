...
description: "Spring 同名重载 @Bean 与 @ConditionalOnProperty 组合的常见陷阱解析；说明默认 Bean 命名规则、重载工厂方法解析机制与条件评估不稳定原因；还原 @Resource 按名失败后按类型回退导致 multiple beans 冲突链路；给出不同方法名 + 显式 @Bean(\"notifyClient\")、@Qualifier/@Resource(name=...) 的稳定写法与排查步骤。"
