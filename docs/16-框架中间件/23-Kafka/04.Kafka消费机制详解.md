...
description: "Offset偏移量双重身份消息位置与消费进度;offset自动/手动同步/异步提交策略对比;顺序消费单分区/Key/自定义分区器三种方案;Spring Kafka批量消费配置与可靠性保障;消费者组协调机制与分区分配策略;Range/RoundRobin/Sticky分配算法解析。"
