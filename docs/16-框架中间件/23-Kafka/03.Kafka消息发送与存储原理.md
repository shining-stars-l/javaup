...
description: "Producer双线程架构主线程与Sender线程分工;拦截器/序列化器/分区器处理流程;RecordAccumulator消息累加器批量发送机制;Partition/Segment分层存储结构;Page Cache/零拷贝/顺序写磁盘高性能原理;稀疏索引加速查找与消息读写完整流程。"
