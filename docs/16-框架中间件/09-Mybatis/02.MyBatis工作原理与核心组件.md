...
title: "MyBatis工作原理与核心组件：MapperProxy、SqlSession、Executor执行器详解"
sidebar_label: "MyBatis工作原理与核心组件"
pagination_label: "MyBatis工作原理与核心组件"
description: "启动阶段配置解析与运行阶段执行;JDK动态代理生成Mapper实现;SqlSession四大核心对象;Executor执行器StatementHandler参数处理;MapperProxy代理机制;spring.factories加载流程。内容进一步围绕MapperProxy、SqlSession、Executor执…"
