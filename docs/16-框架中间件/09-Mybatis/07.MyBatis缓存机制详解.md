...
title: "MyBatis缓存机制：一级缓存、二级缓存、PerpetualCache、namespace缓存详解"
sidebar_label: "MyBatis缓存机制详解"
pagination_label: "MyBatis缓存机制详解"
sidebar_class_name: has-paid-badge
description: "一级缓存SqlSession级别默认开启;二级缓存namespace级别配置;PerpetualCache HashMap实现;缓存命中条件与失效时机;update/insert/delete清空缓存;分布式环境缓存一致性问题。内容进一步围绕namespace缓存、缓存失效等关键主题展开。通过原理拆解、实现步骤与适用…"
