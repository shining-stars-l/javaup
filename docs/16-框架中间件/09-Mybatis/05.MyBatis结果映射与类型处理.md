...
title: "MyBatis结果映射与类型处理：resultMap、association、collection详解"
sidebar_label: "MyBatis结果映射与类型处理"
pagination_label: "MyBatis结果映射与类型处理"
description: "resultType自动映射与resultMap高级映射;列名别名下划线转驼峰;association一对一/collection一对多;嵌套查询与嵌套结果;TypeHandler类型处理器自定义;枚举类型JSON类型处理。内容进一步围绕驼峰映射等关键主题展开。通过原理拆解、实现步骤与适用场景说明相关方案如何落地。"
keywords: ["resultMap", "association", "collection", "TypeHandler", "驼峰映射"]
---
