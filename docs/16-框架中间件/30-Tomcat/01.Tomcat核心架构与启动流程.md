...
description: "Server/Service/Connector/Engine/Host/Context组件层次架构;Bootstrap启动三阶段init/load/start流程;类加载器初始化与Catalina创建;server.xml配置解析与组件初始化;Lifecycle统一生命周期管理状态流转机制。"
