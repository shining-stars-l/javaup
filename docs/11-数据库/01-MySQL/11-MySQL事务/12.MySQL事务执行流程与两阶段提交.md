...
title: "MySQL事务执行流程与两阶段提交：Buffer Pool、Binlog、XID、Redo Log详解"
sidebar_label: "MySQL事务执行流程与两阶段提交"
pagination_label: "MySQL事务执行流程与两阶段提交"
description: "InnoDB事务更新执行流程;Buffer Pool数据页操作;Undo Log与Redo Log写入顺序;两阶段提交(2PC)保证主从一致性;XID关联机制;Binlog三种格式(Statement/Row/Mixed)对比。内容进一步围绕两阶段提交、Buffer Pool、Binlog、XID、Redo Log等…"
keywords: ["两阶段提交", "Buffer Pool", "Binlog", "XID", "Redo Log"]
---
...
INSERT操作的Undo Log类型为`TRX_UNDO_INSERT_REC`，明确标识这是一个插入操作，回滚时需要执行删除。

**Undo Log记录的关键信息**：

```mermaid
graph LR
    A([INSERT Undo Log]) --> B[Undo Type<br/>TRX_UNDO_INSERT_REC]
    A --> C[Undo No<br/>事务内序号]
    A --> D[Table ID<br/>表标识符]
    A --> E[主键信息<br/>len,value列表]
    ...
    style E fill:#E74C3C,stroke:#A93226,stroke-width:2px,color:#fff,rx:10,ry:10
```
...
sequenceDiagram
    ...
    App->>BP: 1. 在内存中准备插入数据
    App->>Undo: 2. 写入Undo Log<br/>记录INSERT操作和主键信息
    Undo-->>App: 写入成功
    
    App->>BP: 3. 在Buffer Pool中插入数据
    App->>Redo: 4. 写入Redo Log (Prepare阶段)<br/>标记事务状态为Prepare
    Redo-->>App: Prepare完成，返回XID
    
    App->>Bin: 5. 写入Binlog<br/>记录INSERT语句或行变更
    Bin-->>App: Binlog刷盘成功
    
    App->>Redo: 6. 写入Redo Log (Commit阶段)<br/>标记事务状态为Commit
    Redo-->>App: 事务提交完成
    
    Note over Undo,Bin: 写入顺序：Undo → Redo(Prepare) → Binlog → Redo(Commit)
```
...
```mermaid
graph TD
    A([开始INSERT操作]) --> B{Undo Log先写入}
    B --> C[记录变更前状态]
    C --> D[执行实际数据修改]
    D --> E{需要回滚?}
    E -->|是| F[根据Undo Log删除记录]
    E -->|否| G[继续提交流程]
    F --> H([回滚完成])
    G --> I([正常提交])
    ...
    style H fill:#95A5A6,stroke:#566573,stroke-width:2px,color:#fff,rx:15,ry:15
...
graph TB
    ...
    B --> C{MySQL崩溃}
    C --> D[Binlog未写入]
    D --> E[主库恢复后<br/>数据已插入stock=500]
    D --> F[从库无此Binlog<br/>数据未插入]
    E --> G([主从不一致])
    F --> G
    
    style A fill:#4A90E2,stroke:#2E5C8A,stroke-width:2px,color:#fff,rx:15,ry:15
    style B fill:#50C878,stroke:#2E7D4E,stroke-width:2px,color:#fff,rx:10,ry:10
    style C fill:#E67E22,stroke:#A04000,stroke-width:2px,color:#fff,rx:10,ry:10
    style D fill:#E74C3C,stroke:#A93226,stroke-width:2px,color:#fff,rx:10,ry:10
    style E fill:#9B59B6,stroke:#6C3483,stroke-width:2px,color:#fff,rx:10,ry:10
    style F fill:#1ABC9C,stroke:#117864,stroke-width:2px,color:#fff,rx:10,ry:10
    style G fill:#D32F2F,stroke:#B71C1C,stroke-width:3px,color:#fff,rx:15,ry:15
```
...
graph TB
    ...
    B --> C{MySQL崩溃}
    C --> D[Redo Log未写入]
    D --> E[主库恢复后<br/>数据未插入缺少Redo Log]
    D --> F[从库执行Binlog<br/>数据已插入stock=500]
    E --> G([主从不一致])
    F --> G
...
```mermaid
graph TD
    A([两阶段提交流程]) --> B[Redo Log Prepare]
    B --> C[Binlog写入]
    C --> D{崩溃点1<br/>Binlog写入前}
    C --> E{崩溃点2<br/>Binlog写入后}
    
    D --> F[恢复检查：<br/>Binlog无XID记录]
    F --> G[回滚事务]
    G --> H([主从都未提交<br/>数据一致])
    
    E --> I[恢复检查：<br/>Binlog有XID记录]
    I --> J[提交事务<br/>Redo Log改为Commit]
    J --> K([主从都已提交<br/>数据一致])
    ...
    style G fill:#E74C3C,stroke:#A93226,stroke-width:2px,color:#fff,rx:10,ry:10
    style J fill:#1ABC9C,stroke:#117864,stroke-width:2px,color:#fff,rx:10,ry:10
    style H fill:#2ECC71,stroke:#1E8449,stroke-width:2px,color:#fff,rx:15,ry:15
...
```mermaid
graph TB
    A([DML操作类型]) --> B[INSERT操作]
    A --> C[UPDATE操作]
    A --> D[DELETE操作]
    
    B --> B1[Undo Type:<br/>TRX_UNDO_INSERT_REC]
    B --> B2[记录内容:<br/>主键信息]
    B --> B3[回滚操作:<br/>DELETE]
    
    C --> C1[Undo Type:<br/>TRX_UNDO_UPD_EXIST_REC]
    C --> C2[记录内容:<br/>修改前的完整行数据]
    C --> C3[回滚操作:<br/>UPDATE回旧值]
    
    D --> D1[Undo Type:<br/>TRX_UNDO_DEL_MARK_REC]
    D --> D2[记录内容:<br/>删除前的完整行数据]
    D --> D3[回滚操作:<br/>INSERT恢复数据]
    
    style A fill:#4A90E2,stroke:#2E5C8A,stroke-width:2px,color:#fff,rx:15,ry:15
    style B fill:#50C878,stroke:#2E7D4E,stroke-width:2px,color:#fff,rx:10,ry:10
    style C fill:#9B59B6,stroke:#6C3483,stroke-width:2px,color:#fff,rx:10,ry:10
    style D fill:#E67E22,stroke:#A04000,stroke-width:2px,color:#fff,rx:10,ry:10
    style B1,B2,B3 fill:#1ABC9C,stroke:#117864,stroke-width:1px,color:#fff,rx:8,ry:8
    style C1,C2,C3 fill:#E74C3C,stroke:#A93226,stroke-width:1px,color:#fff,rx:8,ry:8
    style D1,D2,D3 fill:#F39C12,stroke:#D68910,stroke-width:1px,color:#fff,rx:8,ry:8
```
