...
title: "四层渐进式加载机制揭秘：渐进式披露、按需加载、上下文优化、Token消耗、技能加载机制详解"
sidebar_label: "四层渐进式加载机制揭秘"
pagination_label: "四层渐进式加载机制揭秘"
description: "全面拆解Agent Skills的四层渐进式加载机制，理解Metadata、Instruction、Reference、Script如何分阶段按需进入上下文，以及这套设计背后的Token优化逻辑。内容进一步围绕渐进式披露、按需加载、上下文优化、Token消耗、技能加载机制等关键主题展开。通过原理拆解、实现步骤与适用场…"
...
:::tip 实战项目推荐
渐进式加载解决的是上下文预算和能力选择问题。超级 AI 智能体中的知识路由、动态 Agent 和 Skills 思路有相似目标：先判断需求，再按需加载真正相关的知识和能力。
