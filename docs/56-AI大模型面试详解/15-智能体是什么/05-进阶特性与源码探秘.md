---
slug: /ai-interview/agent/advanced-features
title: "进阶特性与源码探秘：Human in the Loop、HITL、Agent记忆、Graph引擎详解"
sidebar_label: "进阶特性与源码探秘"
pagination_label: "进阶特性与源码探秘"
description: "Agent进阶特性：Human in the Loop人工审批机制、长期记忆持久化方案，以及ReactAgent源码原理深度剖析。内容进一步围绕HITL、Agent记忆、Graph引擎等关键主题展开。通过原理拆解、实现步骤与适用场景说明相关方案如何落地。同时补充常见问题、排查思路、项目实践建议与技术面试要点。"
...
:::tip 实战项目推荐
Agent 的进阶能力要落到安全、可控、可观测和可扩展上。超级 AI 智能体通过动态 Agent、执行阶段记录和链路追踪，把这些源码层概念转成可调试的业务能力。
...
:::caution 持久化记忆的必要性
`MemorySaver`是基于内存的，应用一重启，所有对话历史都没了。生产环境肯定不能这样。持久化记忆的好处：
- 应用重启后对话可继续
- 支持分布式部署（多实例共享记忆）
- 可做数据分析和审计
:::
...
| Saver | 存储方式 | 特点 | 适用场景 |
|-------|---------|------|---------|
| **MemorySaver** | 内存（ConcurrentHashMap） | 速度快，重启丢失 | 开发测试、短期对话、演示 |
| **FileSystemSaver** | 文件系统（JSON 文件） | 简单持久化，单机有效 | 单机部署、数据量小 |
| **MysqlSaver** | MySQL 数据库 | 关系型数据库，支持事务 | 生产环境、已有 MySQL |
| **PostgresSaver** | PostgreSQL 数据库 | 功能强大，JSON 支持好 | 生产环境、已有 PostgreSQL |
...
```plantuml title="ReactAgent底层架构" width="50%" align="left"
@startuml
...
```plantuml title="ReactAgent执行流程" width="50%" align="left"
@startuml
