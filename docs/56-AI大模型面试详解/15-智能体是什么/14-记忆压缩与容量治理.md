...
title: "记忆压缩与容量治理：滑动窗口、摘要蒸馏、token预算、重要性过滤、记忆衰减、context管理详解"
sidebar_label: "记忆压缩与容量治理"
pagination_label: "记忆压缩与容量治理"
description: "当Agent对话越来越长、context快要撑爆时的四种应对策略：滑动窗口的取舍、摘要蒸馏的层次设计、重要性评分筛选、结构化提取降维，以及记忆衰减和遗忘机制的工程实现。内容进一步围绕记忆压缩、token预算、重要性过滤、Prompt Caching、context管理等关键主题展开。通过原理拆解、实现步骤与适用场景说…"
