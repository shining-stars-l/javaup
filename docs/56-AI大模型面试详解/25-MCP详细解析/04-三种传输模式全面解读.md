---
slug: /ai-interview/mcp/transport-modes
title: "三种传输模式全面解读：MCP传输模式、Stdio、SSE、Streamable HTTP详解"
sidebar_label: "三种传输模式全面解读"
pagination_label: "三种传输模式全面解读"
description: "全面解读MCP三种传输模式Stdio、SSE、Streamable HTTP的技术原理、适用场景与选型策略。内容进一步围绕MCP传输模式、MCP选型、智能体通信等关键主题展开。通过原理拆解、实现步骤与适用场景说明相关方案如何落地。同时补充常见问题、排查思路、项目实践建议与技术面试要点。帮助开发者建立完整知识体系，并将…"
...
:::tip 实战项目推荐
传输模式决定 MCP 能力适合放在本地、远程还是流式场景里。超级 AI 智能体关注的是这些外部能力接入后如何被 Agent 编排、观测和治理。
...
:::warning SSE 模式已不推荐使用
SSE 模式存在双端点管理麻烦、长连接容易断开、缺乏自动重连机制等痛点。MCP 官方已在 2025 年 3 月推出 Streamable HTTP 作为替代方案。**新项目不建议选用 SSE 模式**，仅在维护老项目时保留。
:::
...
:::info 会话 ID 机制
`Mcp-Session-Id` 是 Streamable HTTP 模式的重要特性。Client 首次请求时 Server 在响应头中返回会话 ID，后续所有请求携带此 ID，使得即便网络断开重连，Server 也能恢复之前的会话状态。
:::
...
:::tip 选型口诀
**本地用 Stdio，远程用 Streamable HTTP**。SSE 仅在需要兼容旧版本客户端时保留，新项目一律选 Streamable HTTP。
:::
...
:::note 迁移成本极低
从 SSE 迁移到 Streamable HTTP 只需修改 Server 端配置文件和 Client 端的 Server 地址配置。所有工具实现代码无需任何改动，传输层切换对业务完全透明。
:::
