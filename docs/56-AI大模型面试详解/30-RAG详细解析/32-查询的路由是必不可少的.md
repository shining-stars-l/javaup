...
title: "查询的路由是必不可少的：意图识别、查询路由、Query Routing、Text2SQL详解"
sidebar_label: "查询的路由是必不可少的"
pagination_label: "查询的路由是必不可少的"
description: "不是所有用户提问都该走向量检索。闲聊、查订单、模糊提问，每种意图需要不同的处理通道。本文拆解意图识别的三种方案和四条路由通道，附带Text2SQL、问题澄清等完整实现。内容进一步围绕查询路由、Query Routing、Spring AI、LangChain4j、RAG优化等关键主题展开。通过原理拆解、实现步骤与适用…"
...
:::tip 实战项目推荐
查询路由是超级 AI 智能体的重点亮点之一：项目实现了三级知识路由和路由评测，把问题分发到更合适的知识域、检索链路或 Agent 能力中。
...
**先在`application.yaml`中配置好ApiKey**：[参考硅基流动章节中apiKey的设置](/ai-interview/llm-intro/dev-environment)
