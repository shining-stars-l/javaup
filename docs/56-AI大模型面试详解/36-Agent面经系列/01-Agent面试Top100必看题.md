...
description: "我把多场 Agent 面经里的高频追问重新整理成 100 道题，覆盖智能体原理、ReAct、规划、多 Agent、RAG、上下文与记忆、工具调用、MCP、Skills、评测、安全和工程化。每题给出回答抓手与继续追问，适合面试前快速过一遍。"
...
我把最近看到的几场 Agent 面经重新过了一遍，删掉了重复问法，也把面试官常见的追问补在每题后面。小伙伴准备面试时，可以先看“回答抓手”，再用自己的项目经历把它换成具体的数字和故障案例。

这份清单适合做最后一轮自测：只背定义很容易被追问卡住，至少要能说清楚为什么这样设计、出了问题怎么定位、换一种方案会付出什么代价。

:::info 使用方法
先按主题快速浏览 100 题；第二遍只看自己答不顺的题；第三遍把答案改成项目口径。面试官问到“你做过吗”时，优先讲真实做过的部分，没做过就说明调研过什么、准备怎样验证。
:::
...
- 不要从“我对人工智能很感兴趣”开始，直接说做了什么，例如“我负责一个面向运维同学的故障分析 Agent，接入知识库和监控工具”。
...
- 说清自己负责的模块，别把团队成果全算到自己头上。

### 3. 这个项目解决了什么痛点？为什么值得用 Agent？

- 说明原来的人工流程卡在哪一步，最好给出耗时或错误率。
- 只有需要理解自然语言、跨多个步骤和工具的任务，才值得引入 Agent。固定规则能解决的流程，直接用 Workflow 通常更稳。
- 说一个“不适合用 Agent”的反例，能体现取舍。
...
- Python 生态适合模型实验、数据处理和快速接入；Java 在已有微服务、并发治理、权限和监控体系里更顺手。
...
- 如果长流程里节点和状态很多，我会优先选图编排，避免把所有分支塞进一个大 Prompt。
...
- 最后给验证数据：重试次数、P95 延迟、成功率或人工接管比例。
...
### 94. 三万个协程和三万个线程在 8 核 CPU 上有什么区别？

- 线程由操作系统调度，栈和切换成本更高；协程由运行时调度，适合大量 I/O 等待。
- CPU 密集任务最终仍受 8 核限制，协程不会凭空增加计算能力。
- 还要看语言运行时、阻塞调用是否会卡住协程，以及连接池和内存是否撑得住。
...
### 96. 数据库索引和 Agent 项目有什么关系？

- 会话、任务、工具事件和文档版本都需要按租户、状态、时间和任务 ID 查询。
- 联合索引要遵守最左匹配；范围查询、函数操作、隐式类型转换可能让索引失效。
- LIKE 以固定前缀开始时通常可以利用 B+ 树，`%keyword%` 通常不行。

### 97. 如何设计一张存 K 线数据的表？

- 用交易所、交易对、周期和开盘时间组成唯一键，保存 open/high/low/close/volume 等定长字段。
- 按交易对和时间分区或建立联合索引，金额和数量用定点类型，避免浮点误差。
- 写入要幂等，迟到数据要有修订策略，查询默认按时间范围限制。

### 98. 现场让你手撕合并有序链表/数组怎么答？

- 双指针从两个头部向后比较，较小值写入结果，剩余部分直接拼接。
- 时间复杂度 `O(m+n)`，额外空间链表为 `O(1)`，数组视题目是否允许原地修改。
- 先确认空输入、重复值和是否要求稳定顺序，再开始写。

### 99. 二叉树前序遍历和层序遍历怎么写？

- 前序是根、左、右；递归容易写，迭代用栈模拟。
- 层序用队列，每轮记录当前层大小；空间复杂度取决于最大宽度。
- 面试官继续追问时，可以说明空树、单节点和极深树的处理。

### 100. 最长递增子序列怎么讲清楚？

- 经典 `O(n^2)` DP：`dp[i]` 表示以 `i` 结尾的最长长度，枚举前面更小的元素。
- 需要更高性能时，用 tails 数组和二分做到 `O(n log n)`；tails 记录的是各长度下的最小结尾，不等于最终子序列本身。
- 先问清楚是严格递增还是非递减，以及只要长度还是要输出序列。

## 最后给自己的复习清单

我建议小伙伴最后不要继续堆概念，拿自己的项目逐项对照：

- 能不能在 1 分钟内画出请求链路，并指出自己负责的节点？
- 每个工具有没有权限、超时、幂等和审计？
