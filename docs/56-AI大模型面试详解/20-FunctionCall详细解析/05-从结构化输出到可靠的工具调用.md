...
title: "从结构化输出到可靠的工具调用：JSON Schema、Constrained Decoding详解"
sidebar_label: "从结构化输出到可靠的工具调用"
pagination_label: "从结构化输出到可靠的工具调用"
description: "拆解大模型结构化输出的底层保障机制，搞清楚为什么光靠提示词约束JSON不够可靠，JSON Mode和Structured Outputs各自做了什么，以及这些技术如何支撑起可靠的Function Calling。内容进一步围绕JSON Schema、Constrained Decoding、Function Call…"
