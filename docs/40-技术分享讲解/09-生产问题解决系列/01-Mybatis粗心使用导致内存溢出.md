...
title: "Mybatis粗心使用导致内存溢出：核心原理、实现机制、源码分析、工程实践、应用场景详解"
sidebar_label: "Mybatis粗心使用导致内存溢出"
pagination_label: "Mybatis粗心使用导致内存溢出"
description: "服务响应变慢，线程日志也出现Java heap space内存溢出的错误，这个服务属于基础业务服务，出现问题要尽快的排查。内容进一步围绕Mybatis粗心使用导致内存溢出等关键主题展开。通过原理拆解、实现步骤与适用场景说明相关方案如何落地。同时补充常见问题、排查思路、项目实践建议与技术面试要点。帮助开发者建立完整知识…"
---
...
![gc.log：GC前](/img/technologySharing/mybatis/GC前.png)
![gc.log：GC后](/img/technologySharing/mybatis/GC后.jpeg)
![gc.log：Old代](/img/technologySharing/mybatis/Old代.jpeg)
可以很明显的看出gc已经不能把对象进行回收了发生了内存泄露

## hprof文件

![hprof文件](/img/technologySharing/mybatis/hprof文件.jpeg)
...
![hprof文件：mybatis中这条sql的逻辑](/img/technologySharing/mybatis/mybatis中这条sql的逻辑.jpeg)
...
![解决思路：加上条件限制](/img/technologySharing/mybatis/加上条件限制.jpeg)
