---
slug: /architecture/sharding/distributed-database
title: "分布式数据库核心原理与选型实践：TiDB、OceanBase、NewSQL、存算分离详解"
sidebar_label: "分布式数据库核心原理与选型实践"
pagination_label: "分布式数据库核心原理与选型实践"
description: "TiDB/OceanBase/PolarDB/Spanner产品对比;分布式事务2PC与共识算法实现;存储计算分离与数据分片策略;CAP理论与NewSQL技术选型决策。内容进一步围绕分布式数据库、存算分离等关键主题展开。通过原理拆解、实现步骤与适用场景说明相关方案如何落地。同时补充常见问题、排查思路、项目实践建议与技…"
keywords: ["分布式数据库", "TiDB", "OceanBase", "NewSQL", "存算分离"]
---
# 分布式数据库核心原理与选型实践
