---
slug: /java/base/interfaces-abstract-classes
title: "接口与抽象类：多继承、菱形问题、默认方法、模板方法模式、组合优于继承、Interface详解"
sidebar_label: "接口与抽象类"
pagination_label: "接口与抽象类"
description: "全面对比接口与抽象类的设计差异,包括方法定义、成员变量、构造器、继承/实现规则;通过消息服务案例展示接口+抽象类的组合使用最佳实践;图解Java不支持多继承的菱形问题及Java 8默认方法的冲突解决;讲解组合优于继承的设计原则与实战建议。内容进一步围绕模板方法模式、Interface、Abstract等关键主题展开。"
keywords: ["接口", "抽象类", "多继承", "菱形问题", "默认方法", "模板方法模式", "组合优于继承", "Interface", "Abstract"]
---
...
在同等可行的情况下,更推荐使用组合而不是继承。组合通过把对象作为成员进行拼装来获得能力,更利于封装与扩展;继承适合建模稳定的层次结构,但会放大父类改动对子类的影响。
...
graph LR
    ...
    classDef base fill:#4A90E2,stroke:none,color:#fff
    classDef sub fill:#50C878,stroke:none,color:#fff
...
graph LR
    ...
    classDef car fill:#E85D75,stroke:none,color:#fff
    classDef part fill:#4A90E2,stroke:none,color:#fff
    
    class Car car
    class Engine,Wheels,Brakes part
```

**差异与选择**
- 白盒复用:继承暴露并依赖父类实现,子类行为受父类改动影响
- 黑盒复用:组合通过接口拼装能力,更易替换实现并做扩展
- 语义建模:确实存在稳定的“是一个”关系用继承;表达“有一个”关系用组合
- 变更敏感:不确定将来是否需要替换实现时,优先用组合
...
class SportsCar extends Vehicle {
    @Override
    void move() { System.out.println("高速行驶"); }
}

// 组合建模:按需替换部件
interface Engine { void throttle(); }
interface Brake { void apply(); }

class TurboEngine implements Engine { public void throttle() { System.out.println("涡轮增压"); } }
class DiskBrake implements Brake { public void apply() { System.out.println("碟刹制动"); } }
class DrumBrake implements Brake { public void apply() { System.out.println("鼓刹制动"); } }

class Car {
    private Engine engine;
    private Brake brake;
    
    Car(Engine engine, Brake brake) {
        this.engine = engine;
        this.brake = brake;
    }
    
    void accelerate() { engine.throttle(); }
    void stop() { brake.apply(); }
    
    void replaceBrake(Brake newBrake) { this.brake = newBrake; }
}

// 使用:组合支持运行时替换能力
Car car = new Car(new TurboEngine(), new DiskBrake());
car.accelerate();
car.stop();
car.replaceBrake(new DrumBrake());
car.stop();
```
