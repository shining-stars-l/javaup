...
title: "什么是RAG和向量数据库：Embedding向量化、ANN检索、余弦相似度、语义召回详解"
sidebar_label: "什么是RAG和向量数据库"
pagination_label: "什么是RAG和向量数据库"
description: "RAG与向量数据库核心原理解析，讲解Embedding向量化、相似度计算、ANN检索机制以及检索结果与大模型生成的拼接方式，帮助构建可追溯的知识问答链路。内容进一步围绕余弦相似度、语义召回、知识增强生成、问答准确率等关键主题展开。通过原理拆解、实现步骤与适用场景说明相关方案如何落地。同时补充常见问题、排查思路、项目实…"
...
![向量表示示意图 1](/img/damai-ai/什么是RAG和向量数据库/1.png)


通过向量表示，模型可以量化语义相似度——两个内容的向量越接近，它们的含义就越相近。

![向量表示示意图 2](/img/damai-ai/什么是RAG和向量数据库/2.png)

![向量表示示意图 3](/img/damai-ai/什么是RAG和向量数据库/3.jpg)

![向量表示示意图 4](/img/damai-ai/什么是RAG和向量数据库/4.jpg)


#### 度量：常用余弦相似度（Cosine Similarity）、欧氏距离（Euclidean Distance）
![度量：常用余弦相似度（Cosine Similarity）、欧氏距离（Euclidean Distance）示意图 5](/img/damai-ai/什么是RAG和向量数据库/5.jpg)
...
![RAG 工作流程示意图 6](/img/damai-ai/什么是RAG和向量数据库/6.webp)

_用户问题经过嵌入检索模块，从向量数据库中获取相关内容，然后与问题一同传递给生成模型（LLM），得到最终回答。图中展示了文档拆分、向量化、检索、重排序等步骤_

![RAG 工作流程示意图 7](/img/damai-ai/什么是RAG和向量数据库/7.png)
